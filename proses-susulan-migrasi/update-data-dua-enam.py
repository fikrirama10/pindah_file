...
def main():
    ...
    query = "SELECT IDPERUSAHAANFINAL FROM vw_datalembaga WHERE PROSES LIKE '%2/6%'"
    ...
    for row in result:
        ...
        tahapan = "lengkap registrasi dan siap mengajukan ulop"
        query2 = "UPDATE jenis_penyiaran SET tahapan=%s, id_tahapan=%s, status_registrasi = %s,updated_at=%s WHERE idperusahaanfinal_origin = %s"
        cursor.execute(query2, (tahapan,2,3,update_at,row[0]))
        connection.commit()
