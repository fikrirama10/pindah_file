...
def lampiranfile():
    ...
    sql = "SELECT DISTINCT jenis_penyiaran.id_asal, jenis_penyiaran.idperusahaanfinal_origin FROM jenis_penyiaran"
    cursor.execute(sql)
    ...
    for i in list_:
        ...
        if result:
            for j in result:
                ...
                elif field_name == 'LATAR_BELAKANG':
                    # update field file_fotokopi_npwp di tabel lampiran_penyiaran berdasarkan id_perusahaanfinal_origin
                    sql2 = "UPDATE lampiran_penyiaran SET file_latar_belakang = %s WHERE idperusahaanfinal_origin = %s"
                    cursor2.execute(sql2, (file_name, idperusahaanfinal_origin, ))
                    connection2.commit()
                    print("record updated.", file_name, idperusahaanfinal_origin)
                    
                elif field_name == 'MAKSUD_PENDIRIAN':
                    # update field file_fotokopi_npwp di tabel lampiran_penyiaran berdasarkan id_perusahaanfinal_origin
                    sql2 = "UPDATE lampiran_penyiaran SET file_maksud_dan_tujuan = %s WHERE idperusahaanfinal_origin = %s"
                    cursor2.execute(sql2, (file_name, idperusahaanfinal_origin, ))
                    connection2.commit()
                    print("record updated.", file_name, idperusahaanfinal_origin)
                
                elif field_name == 'VISI':
                    # update field file_fotokopi_npwp di tabel lampiran_penyiaran berdasarkan id_perusahaanfinal_origin
                    sql2 = "UPDATE lampiran_penyiaran SET file_visi = %s, file_visi_misi = %s  WHERE idperusahaanfinal_origin = %s"
                    cursor2.execute(sql2, (file_name,file_name, idperusahaanfinal_origin, ))
                    connection2.commit()
                    print("record updated.", file_name, idperusahaanfinal_origin)
                
                elif field_name == 'MISI':
                    # update field file_fotokopi_npwp di tabel lampiran_penyiaran berdasarkan id_perusahaanfinal_origin
                    sql2 = "UPDATE lampiran_penyiaran SET file_misi = %s WHERE idperusahaanfinal_origin = %s"
                    cursor2.execute(sql2, (file_name, idperusahaanfinal_origin, ))
                    connection2.commit()
                    print("record updated.", file_name, idperusahaanfinal_origin)
