...
def main():
    ...
    query = "SELECT IDPERUSAHAANFINAL,date_created FROM vw_datalembaga "
    ...
    for row in result:
        ...
        query2 = "UPDATE jenis_penyiaran SET tgl_submit=%s,updated_at=%s WHERE idperusahaanfinal_origin = %s"
        cursor.execute(query2, (row[1],update_at,row[0]))
        connection.commit()
