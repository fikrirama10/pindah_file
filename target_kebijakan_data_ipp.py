...
def cek_ipp_prinsip(id):
    ...
    cursor = db.cursor()
    sql = "SELECT no_ipp_prinsip,tgl_ipp_prinsip,tgl_perpanjangan FROM tbl_ipp_prinsip WHERE id_perusahaan='"+str(id)+"' ORDER BY id_ipp_prinsip ASC"
    cursor.execute(sql)
    results = cursor.fetchone()
    if results is None:
        pass
    else:
        #update tahapan_kebijakan set no_ipp_prinsip = 'no_ipp_prinsip', tgl_ipp_prinsip = 'tgl_ipp_prinsip', tgl_perpanjangan_prinsip = 'tgl_perpanjangan' where idperusahaan = 'id'
        db2 = connect_db()
        cursor2 = db2.cursor()
        sql = "UPDATE tahapan_kebijakan SET no_ipp = '"+str(results[0])+"', tgl_ipp = '"+str(results[1])+"', tgl_perpanjangan = '"+str(results[2])+"' WHERE id_perusahaan = '"+str(id)+"'"
        cursor2.execute(sql)
        db2.commit()
...
def cek_nama_perusahaan(nama):
    ...
    if results is None:
        ...
        #update IDPERUSAHAAN DI TABEL tahapan_kebijakan
        db2 = connect_db()
        cursor2 = db2.cursor()
...
def cek_ipp_tetap_zzhitoris(id):
    db = connect_db_golive()
    cursor = db.cursor()
    sql = "SELECT no_ipp_tetap,tgl_ipp_tetap,tgl_perpanjangan FROM zzhistoris_tbl_ipp_tetap WHERE id_perusahaan='"+str(id)+"' ORDER BY id_ipp_tetap ASC"
    cursor.execute(sql)
    results = cursor.fetchone()
    if results is None:
        ...
        #update tahapan_kebijakan set no_ipp_tetap = 'no_ipp_tetap', tgl_ipp_tetap = 'tgl_ipp_tetap', tgl_perpanjangan = 'tgl_perpanjangan' where idperusahaan = 'id'
        db2 = connect_db()
        cursor2 = db2.cursor()
        sql = "UPDATE tahapan_kebijakan SET no_ipp = '"+str(results[0])+"', tgl_ipp = '"+str(results[1])+"', tgl_perpanjangan = '"+str(results[2])+"' WHERE id_perusahaan = '"+str(id)+"'"
        cursor2.execute(sql)
...
def cek_ipp_tetap(id):
    ...
    sql = "SELECT no_ipp_tetap,tgl_ipp_tetap,tgl_perpanjangan FROM tbl_ipp_tetap WHERE id_perusahaan='"+str(id)+"' ORDER BY id_ipp_tetap ASC"
    cursor.execute(sql)
    results = cursor.fetchone()
    if results is None:
...
def cek_status_evaluasi(id):
    ...
    cursor = db.cursor()
    # sql = "SELECT no_ipp_tetap,tgl_ipp_tetap,tgl_perpanjangan FROM tbl_ipp_tetap WHERE id_perusahaan='RS201602390' ORDER BY id_ipp_tetap ASC"
    sql = "SELECT keputusan_perpanjang_subdit,tgl_ba_subdit FROM vw_berita_acata WHERE IDPERUSAHAANFINAL='"+str(id)+"' AND tgl_ba_subdit IS NOT NULL"
    cursor.execute(sql)
    results = cursor.fetchone()
    if results == None:
        pass
...
def cek_status_evaluasi(id):
    ...
    if results == None:
        ...
        sql = "UPDATE tahapan_kebijakan SET status_evaluasi = '"+str(results[0])+"', tgl_evaluasi = '"+str(results[1])+"' WHERE id_perusahaan = '"+str(id)+"'"
        cursor2.execute(sql)
        db2.commit()
        return id
...
def get_id_jenis_penyiaran(id):
    db = connect_db()
    cursor = db.cursor()
    sql = "SELECT id FROM jenis_penyiaran WHERE idperusahaanfinal_origin = '"+str(id)+"'"
    cursor.execute(sql)
    results = cursor.fetchone()
    if results is None:
        pass
        ...
        db2 = connect_db()
        cursor2 = db2.cursor()
        sql = "UPDATE tahapan_kebijakan SET id_jenis_penyiaran = '"+str(results[0])+"' WHERE id_perusahaan = '"+str(id)+"'"
        cursor2.execute(sql)
        db2.commit()
    return results
...
def start_():
    db = connect_db()
    cursor = db.cursor()
    sql = "SELECT id_perusahaan,nm_perusahaan FROM tahapan_kebijakan WHERE id_perusahaan IS NOT NULL"
    cursor.execute(sql)
